...
export default function AdminLoginPage() {
  ...
  function fillDemoCredentials() {
    setValue("email", MOCK_ADMIN.email, { shouldValidate: true });
    setValue("password", MOCK_ADMIN.password, { shouldValidate: true });
  }
    ...
    <main className="min-h-screen bg-zinc-50 font-[Inter,ui-sans-serif,system-ui,sans-serif] text-zinc-900 lg:flex">
      <aside className="relative hidden w-[45%] flex-col bg-zinc-900 px-10 py-9 lg:flex">
        <div className="flex items-center gap-3">
          <div className="flex size-9 items-center justify-center rounded-lg bg-zinc-800 text-lg text-white">◈</div>
          <span className="text-lg font-semibold text-white">NexGen</span>
        </div>
        <div className="flex flex-1 items-center">
          <div className="max-w-105">
            <h1 className="text-3xl font-semibold leading-snug text-white">Built for people who run the store.</h1>
            <p className="mt-3 text-sm leading-6 text-zinc-400">Everything you need to manage products, orders, and your team.</p>
            <div className="mt-8 space-y-4">
              {["Full order & inventory control", "Real-time notifications", "Revenue analytics at a glance"].map((feature) => (
                <div key={feature} className="flex items-center gap-3 text-sm text-zinc-300">
                  <span className="flex size-6 shrink-0 items-center justify-center rounded-full bg-zinc-700 text-xs text-white">✓</span>
                  <span>{feature}</span>
                </div>
              ))}
            </div>
      ...
      <section className="flex min-h-screen flex-1 items-center justify-center bg-white px-5 py-10 md:px-8">
        <div className="w-full max-w-100">
          <div>
            <span className="inline-flex rounded-full bg-zinc-100 px-3 py-1 text-xs font-medium text-zinc-600">Admin access only</span>
          ...
          <form className="mt-8 space-y-5" onSubmit={handleSubmit(onSubmit)} noValidate>
            <div>
              <label htmlFor="admin-email" className="mb-1.5 block text-sm font-medium text-zinc-700">Email address</label>
              <input
                id="admin-email"
                type="email"
                {...register("email")}
              ...
              />
              {errors.email ? (
                <p id="admin-email-error" role="alert" className="mt-1 text-xs text-red-500">{errors.email.message}</p>
              ) : null}
            </div>
            <div>
                ...
                >
                  {showPassword ? (
                    <svg viewBox="0 0 24 24" fill="none" aria-hidden="true" className="size-5">
                      <path d="M3 3l18 18M10.6 10.6A2 2 0 0012 14a2 2 0 001.4-.6M9.9 5.2A9.8 9.8 0 0112 5c5 0 8.5 4 9.5 6.6a10.8 10.8 0 01-2.1 3.1M6.1 6.6a11.3 11.3 0 00-3.6 5C3.5 14.1 7 18 12 18c1 0 2-.2 2.9-.5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  ) : (
                    <svg viewBox="0 0 24 24" fill="none" aria-hidden="true" className="size-5">
                      <path d="M2.5 12S6 5 12 5s9.5 7 9.5 7-3.5 7-9.5 7-9.5-7-9.5-7z" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
              ...
              {errors.password ? (
                <p id="admin-password-error" role="alert" className="mt-1 text-xs text-red-500">{errors.password.message}</p>
              ) : authError ? (
                <p role="alert" className="mt-1 text-xs text-red-500">{authError}</p>
              ) : null}
            </div>
