...
export function UsersListPage() {
  ...
  function handleToggleBan(user: AdminUser) {
    updateUserMutation.mutate({ user, payload: { isBanned: !user.isBanned } });
  }

  function handlePromote(user: AdminUser) {
    updateUserMutation.mutate({ user, payload: { role: "ADMIN" } });
  }
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Manage account access, roles, and review customer activity.
          </p>
        </div>
      </div>
            ...
            <table className="w-full min-w-[900px] text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {users.map((user) => (
                  <tr
                    key={user.id}
                    className="hover:bg-slate-50/50 group transition duration-200"
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1">
                        <span
                          className={`inline-flex w-fit rounded-full px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wider ${roleStatusClass[user.role]}`}
                        >
                          {user.role}
                        </span>
                        {user.isSuperAdmin && (
