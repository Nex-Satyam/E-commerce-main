...
const activeImageUrl =
  typeof activeImage === "string"
    ? activeImage
    : (activeImage && typeof activeImage === "object" && "url" in activeImage)
  ...
  {product.images?.map((image, index) => {
    const imgUrl = typeof image === "string" ? image : (image as any)?.url || "/placeholder.png";
    const imgAlt = typeof image === "string"
      ? `${product.name} preview ${index + 1}`
      : (image as any)?.altText || `${product.name} preview ${index + 1}`;
    const imgId = typeof image === "string" ? index : (image as any)?.id || index;
    return (
      <button
        key={`${product.slug}-${imgId}`}
        className={cn(
...
<p className="eyebrow">{typeof product.category === "object" && product.category ? (product.category as any).name : product.category}</p>
            <h1>{product.name}</h1>
